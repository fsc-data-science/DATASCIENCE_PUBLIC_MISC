...
CREATE OR REPLACE TABLE datascience_public_misc.eclipse_analytics.daily_program_outer_tx (
    day_ TIMESTAMP,
    program_id VARCHAR,
...
CALL datascience_public_misc.eclipse_analytics.update_daily_program_outer_tx();

-- Step 4: Define the procedure with a 2-day lookback period
CREATE OR REPLACE PROCEDURE datascience_public_misc.eclipse_analytics.update_daily_program_outer_tx()
RETURNS STRING
LANGUAGE SQL
EXECUTE AS CALLER
AS
$$
BEGIN
    MERGE INTO datascience_public_misc.eclipse_analytics.daily_program_outer_tx AS target
    USING (
        WITH outer_events_tx AS (
            SELECT 
                ...
                END AS program_name,
                COUNT(DISTINCT tx_id) AS n_outer_tx
            FROM eclipse.core.fact_events i 
            LEFT JOIN eclipse.core.dim_labels l ON LOWER(i.program_id) = l.address
            WHERE day_ >= COALESCE(
                DATEADD(day, -2, (SELECT MAX(day_) FROM datascience_public_misc.eclipse_analytics.daily_program_outer_tx)),
                '1970-01-01'
            )
    ...
    WHEN MATCHED THEN
        UPDATE SET 
            ...
            target.n_outer_tx = source.n_outer_tx
    WHEN NOT MATCHED THEN
        INSERT (day_, program_id, program_name, n_outer_tx)
        VALUES (source.day_, source.program_id, source.program_name, source.n_outer_tx);

    RETURN 'Daily program outer transactions updated successfully';
END;
$$;


-- Create task to update outer transactions every 12 hours
CREATE OR REPLACE TASK datascience_public_misc.eclipse_analytics.update_daily_program_outer_tx_task
...
ALTER TASK datascience_public_misc.eclipse_analytics.update_daily_program_outer_tx_task RESUME;
...
GRANT ALL PRIVILEGES ON TABLE datascience_public_misc.eclipse_analytics.daily_program_outer_tx TO ROLE INTERNAL_DEV;
...
GRANT SELECT ON TABLE datascience_public_misc.eclipse_analytics.daily_program_outer_tx TO ROLE VELOCITY_ETHEREUM;