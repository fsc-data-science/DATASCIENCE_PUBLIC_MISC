...
CREATE OR REPLACE TABLE datascience_public_misc.near_analytics.here_near_in_action (
    day_ DATE,
    near_bought FLOAT,
...
CALL datascience_public_misc.near_analytics.update_here_near_in_action();

-- Step 4: Define the procedure with a 90-day lookback period
CREATE OR REPLACE PROCEDURE datascience_public_misc.near_analytics.update_here_near_in_action()
RETURNS STRING
LANGUAGE SQL
EXECUTE AS CALLER
AS
$$
BEGIN
    MERGE INTO datascience_public_misc.near_analytics.here_near_in_action AS target
    USING (
        WITH active_users AS (
            SELECT DISTINCT
                CASE 
                    WHEN tx_signer IN ('relay.tg', '0-relay.hot.tg') THEN tx_receiver 
                    ELSE tx_signer 
                END AS user_
            FROM near.core.fact_transactions 
            WHERE (
                -- HERE wallet user signs a tx themselves
                ENDSWITH(tx_signer, '.tg')
                OR 
                -- HERE relay user
                tx_signer IN (SELECT here_relay_user as tx_signer FROM datascience_public_misc.near_analytics.here_relay_usage)
                )
        ...
        near_dex_buys AS (
            ...
            AND (
                ...
                OR trader IN ('relay.tg', '0-relay.hot.tg')
            )
            GROUP BY 1
        ),
        ...
        near_lending_deposits AS (
            ...
            AND (
                ...
                OR sender_id IN ('relay.tg', '0-relay.hot.tg')
            )
            GROUP BY 1
        ),
...
ALTER TABLE datascience_public_misc.near_analytics.here_near_in_action
CLUSTER BY (day_);
...
GRANT ALL PRIVILEGES ON TABLE datascience_public_misc.near_analytics.here_near_in_action TO ROLE INTERNAL_DEV;
GRANT ALL PRIVILEGES ON TABLE datascience_public_misc.near_analytics.here_near_in_action TO ROLE VELOCITY_ETHEREUM;

-- Create task to update metrics every 12 hours
CREATE OR REPLACE TASK datascience_public_misc.near_analytics.update_here_near_in_action_task
...
-- Resume the task
ALTER TASK datascience_public_misc.near_analytics.update_here_near_in_action_task RESUME;
